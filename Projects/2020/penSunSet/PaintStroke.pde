...
float MAXBCHANGE = 0; // 8
float strokeLength = 80;
...
public class PaintStroke {
  ...
  //a line constructor
  public PaintStroke(float x1_, float y1_, float x2_, float y2_) {
    x1 = x1_;
    y1 = y1_;
    x2 = x2_;
    y2 = y2_;
  ...
  //a curve constructor
  public PaintStroke(float x1_, float y1_, float x2_) {
    x1 = x1_;
    y1 = y1_;
    x2 = x2_;
  ...
  public PaintStroke() {
    this(random(-HELP, width + HELP), random(-HELP, height + HELP), random(-HELP, width + HELP), random(-HELP, height + HELP));
  }
