...
//loading images
String filename = "bigFaceWhite.png";
PImage img;
float[] imageBri;
...
void setup() { 
  ...
  colors.add(color(265, 99, 99));
  colors.add(color(181, 99, 99));
  colors.add(color(181, 8, 99));
...
void picSetup() {
  ...
  loadPixels();
  imageBri = new float[pixels.length];
  for (int i = 0; i < pixels.length; i++) {
    imageBri[i] = brightness(pixels[i]);
  }
}
