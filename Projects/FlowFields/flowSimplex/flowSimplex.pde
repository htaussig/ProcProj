...
//higher values for more variation
float inc = 0.155;
float zoff = 0;
float zInc = 0.00023;
float scl = 9;
...
void draw() {
  ...
  for (int y = 0; y < rows; y++) {
    ...
    for (int x = 0; x < cols; x++) {
      ...
      float n = getNoiseValue(xoff, yoff, zoff);
      n = map(n, -1, 1, 0, TWO_PI);
      //n *= angleMult;
      float angle = n;
...
float getNoiseValue(float x, float y, float zoff) {
  ...
  float u = map(x, 0, cols - 1, 0, TWO_PI);
  float xoff = x;
  float yoff = y;
  
  float v = map(y, 0, rows - 1, 0, TWO_PI);
  
  
  float n = (float) noise(xoff / 1, yoff / 1, zoff / 1);
  //float n = (float) noise.eval(xoff / 1, yoff / 1, zoff / 1);
  //////THIS IS THE LINE YOU NEED FOR SIMPLEX
