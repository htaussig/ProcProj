...
class Vehicle {
  ...
  public Vehicle tryReproduce(){
    Vehicle partner = getPartner();
    if(getPartner() != null){
      //overlap genetics to sexually reproduce
      return reproduce(partner);
    }
  }
  
  private Vehicle getPartner(){
    PVector closest
    
    return null;
  }

  private PVector getClosest(ArrayList<PVector> list, float perception){
  ...
  public PVector move(ArrayList<PVector> list, float perception) {
    PVector closest = getClosest(list, perception);
