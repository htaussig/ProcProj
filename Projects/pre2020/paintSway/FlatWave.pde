public class FlatWave {
  ...
  ArrayList<PVector> display(){
    ...
    ArrayList<PVector> pat = new ArrayList<PVector>();
    
    pat.add(new PVector(0, flat.getPointStand(0) + height / 2));
    
    for(int x = 0; x < points.size() - 1; x++){
      //point(x, points.get(x) + height / 2);
      float y = flat.getPointStand(x + 1) + height / 2;
      pat.add(new PVector(x + 1, y));
      line(x, flat.getPointStand(x) + height / 2, x + 1, y);
    }
